...
5. Jump: rising edge of `jump` while grounded, not punching, not blocking → `vy = -8`, `grounded = false`, `jumpTicks = 0`, emit `JUMP`. Holding jump does not re-jump on landing.
6. Physics per tick: `x += vx`; if airborne `vy += 8/30`, `y += vy`, `jumpTicks++`; landing when `y >= 430` → `y = 430`, `vy = 0`, `grounded = true`, `jumpTicks = 0`. Apex is 120 px at tick 30.
...
- jump apex ≈ 120 at tick 30, lands by tick 61, `jumpTicks` resets
