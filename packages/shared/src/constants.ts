...
// Owner 2026-09-12: platforms and chaos add a third rack, same 180 px span, centred (over chaos's middle ground segment)
// and 100 px above the side racks (reachable from either rack with a jump, apex ~151 px; not straight from the roof).
const THREE_RACKS = [...RACK_PLATFORMS, { x0: 390, x1: 570, y: 230 }];
export const MAPS: Record<MapId, { label: string; ground: { x0: number; x1: number }[]; platforms: { x0: number; x1: number; y: number }[] }> = {
  ...
  platforms: { label: "Platforms", ground: [{ x0: 0, x1: 960 }], platforms: THREE_RACKS },
  chaos: { label: "Chaos", ground: GAPS_GROUND, platforms: THREE_RACKS },
};
