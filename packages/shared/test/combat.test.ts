...
describe("boxes", () => {
  ...
  it("punch hitbox only on active ticks, both facings", () => {
    ...
    f.action = { kind: "punch", arm: "L", elapsed: BALANCE.PUNCH_STARTUP, landed: false };
...
describe("punch resolution", () => {
  ...
  it("a hit cancels the target's punch", () => {
    const s = fighting(60); s.fighters[1].action = { kind: "punch", arm: "R", elapsed: BALANCE.PUNCH_STARTUP + BALANCE.PUNCH_ACTIVE, landed: false }; // in recovery
  ...
  it("jump i-frames: not hit on jumpTicks 3..10, hit on 2 and 11", () => {
    const hitAt = (jt: number): boolean => {
      ...
      s.fighters[0].action = { kind: "punch", arm: "L", elapsed: BALANCE.PUNCH_STARTUP - 1, landed: false };
