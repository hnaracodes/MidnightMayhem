...
| 35 | InputFrame | Frozen six booleans (19) | Gains `special: boolean` (laser) and `item: ItemId \| null` (held object). Protocol v2. **2026-09-12 13:00: gains `chop`, `sweep` (sword slashes), protocol v3 (9.10).** | Owner asked for both; one frame keeps the seq/heartbeat path and the sim's edge detection | |
...
| 52 | Items on camera | Two-item loadout gates equipping (8.01); banana arcs like the molotov; sword = chop/sweep gestures, punch plain (9.10) | **Any held object equips** (loadout no longer gates, equip retries each tick); **throwables are use-only** (a thrust throws, no charge); **the peel slides 300 px along the floor and a slip puts the fighter down for 1.5 s**; **the sword is swung with the punch** (SWORD_REACH / SWORD_DAMAGE, 18 px blade), chop / sweep gestures off behind a flag; **calibration survives a lost body and is remembered per browser** | Owner 2026-09-12 13:30–14:10 after the camera test: phone detected but never equipped (not in the loadout); slash gestures unreliable; "the molotov action is really good — make the others the same" | |
